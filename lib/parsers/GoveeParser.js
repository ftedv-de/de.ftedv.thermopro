...
const MODEL_FORMATS = Object.freeze({
  ...
  H5104: '0001-3byte',
  H5105: '0001-3byte',
  H5106: '0001-air-quality',
  H5108: '0001-3byte',
  H5110: '0001-3byte',
  H5112: '0001-dual-probe',
  H5174: '0001-3byte',
  H5177: '0001-3byte',
  H5178: '0001-dual-sensor',
  H5179: 'auto-h5179',
});
...
function getModel(localName = '') {
  const match = String(localName).toUpperCase().match(/H\d{4}/);
...
function isPlausibleMeasurement(measurement) {
  ...
  if (measurement.temperature !== undefined) {
    if (!Number.isFinite(measurement.temperature) || measurement.temperature < -50 || measurement.temperature > 100) return false;
  }
  ...
  if (measurement.pm25 !== undefined) {
    if (!Number.isInteger(measurement.pm25) || measurement.pm25 < 0 || measurement.pm25 > 999) return false;
  }
  return measurement.temperature !== undefined || measurement.pm25 !== undefined;
}
...
function decodeH5106(data) {
  if (!data || data.length < 6) return null;
  let combined = data.readUInt32BE(2);
  const negative = Boolean(combined & 0x80000000);
  combined &= 0x7fffffff;

  let temperature = Math.floor(combined / 1000000) / 10;
  if (negative) temperature *= -1;
  const humidity = Math.floor(((combined % 1000000) / 1000)) / 10;
  const pm25 = combined % 1000;

  const result = { temperature, humidity, pm25 };
  return isPlausibleMeasurement(result) ? result : null;
}

function decodeH5112(data) {
  if (!data || data.length < 8) return null;
  const measurement = decode3Byte(data, 2);
  if (!measurement) return null;

  const probeMarker = data.readUInt8(7);
  let sensorId = probeMarker;
  if (probeMarker === 0x41) sensorId = 1;
  else if (probeMarker === 0x82) sensorId = 2;

  return {
    ...measurement,
    sensorId,
    probe1Temperature: sensorId === 1 ? measurement.temperature : undefined,
    probe2Temperature: sensorId === 2 ? measurement.temperature : undefined,
  };
}

function decodeH5178(data) {
  if (!data || data.length < 9) return null;
  const measurement = decode3Byte(data, 3);
  if (!measurement) return null;
  return {
    ...measurement,
    sensorId: data.readUInt8(2),
  };
}
...
function decodeFrame(frame, model = null) {
  ...
  if (frame.companyId === COMPANY_IDS.EC88) {
    ...
    if (data.length === 5) return decode3Byte(data, 1);
    if (data.length === 6 || data.length === 8) return decode4ByteLe(data, 1);
    return null;
  }

  if (frame.companyId === COMPANY_IDS.ID_0001) {
    if (format === '0001-air-quality') return decodeH5106(data);
    if (format === '0001-dual-probe') return decodeH5112(data);
    if (format === '0001-dual-sensor') return decodeH5178(data);
    if (format === '0001-3byte') return decode3Byte(data, 2);

    // Safe format inference for unknown model names. The special packet lengths
    // are handled before the generic 3-byte climate format.
    if (data.length === 9) return decodeH5178(data);
...
function findDecodedFrame(adv) {
  ...
  for (const frame of findFrames(adv)) {
    ...
    if (measurement) return { frame, measurement, model, format: model ? MODEL_FORMATS[model] : null };
  }
  return null;
}
...
function parse(adv) {
  ...
  const { frame, measurement, format } = decodedFrame;
  const localName = getLocalName(adv);
  ...
  const protocol = frame.companyId === COMPANY_IDS.EC88
    ...
    : frame.companyId === COMPANY_IDS.ID_0001
      ? 'govee-climate-0001'
      : 'govee-climate-8801';
  const baseName = model === 'Climate Sensor' ? 'Govee Climate Sensor' : `Govee ${model}`;
  ...
  return {
    ...
    model,
    name: baseName,
    displayName: `${baseName} ${suffix}`.trim(),
