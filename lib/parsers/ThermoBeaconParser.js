...
function match(adv) {
  if (!hasThermoBeaconService(adv)) return { confidence: 0, reason: 'service-mismatch' };
  if (getCandidateDataPackets(adv).length === 0) return { confidence: 0, reason: 'payload-mismatch' };

  if (isThermoBeaconName(adv)) {
    return { confidence: 100, reason: 'thermobeacon-name-service-payload' };
  }

  return { confidence: 95, reason: 'thermobeacon-service-payload' };
}
