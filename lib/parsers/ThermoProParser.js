...
function getThermoProPayload(adv) {
  for (const entry of getManufacturerDataEntries(adv)) {
    ...
    // Known devices usually send 0/1/2 here, but do not reject otherwise valid
    // TP... C2 devices solely because a firmware uses another battery marker.
...
function match(adv) {
  ...
  if (!payload) return { confidence: 0, reason: 'payload-mismatch' };

  const localName = getLocalName(adv);
  const model = getModelFromLocalName(localName);

  if (model && KNOWN_MODELS.includes(model)) {
    return { confidence: 95, reason: 'known-thermopro-c2-name-payload' };
  }

  if (model && model.startsWith('TP')) {
    return { confidence: 85, reason: 'generic-tp-c2-name-payload' };
  }

  return { confidence: 55, reason: 'generic-c2-payload' };
}

function canHandle(adv) {
  return match(adv).confidence >= 55;
}
