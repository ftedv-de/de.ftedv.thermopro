...
module.exports = class BaseClimateDevice extends Homey.Device {
  ...
  async performInitialGattRead() {
    // Optional hook. Devices that enable GATT initialization override this method
    // and return true on success or false for a recoverable communication failure.
    return false;
  }
  ...
  async onAdded() {
    ...
    try {
      ...
      this.homey.app.debug('Could not store initial GATT attempt state', {
        ...this.getLogContext(),
        error: err?.message,
      });
    }
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try {
        succeeded = await this.homey.app.runBleOperation(() => this.performInitialGattRead()) === true;
      } catch (err) {
        // SDK calls can still reject unexpectedly. Treat a failed optional interview as
        // recoverable and keep it out of the normal app log.
        this.homey.app.debug('Unexpected initial GATT read error', {
          ...this.getLogContext(),
          attempt,
          attempts,
          error: err?.message,
      ...
      if (succeeded) {
        this.homey.app.debug('Initial GATT read succeeded', {
          ...this.getLogContext(),
          attempt,
        });
        return;
      ...
      this.homey.app.debug('Initial GATT read attempt did not succeed', {
        ...this.getLogContext(),
        attempt,
        attempts,
      });
    ...
    this.homey.app.debug(`Initial ${this.getLogPrefix()} GATT read was not completed`, {
      ...this.getLogContext(),
      attempts,
    });
  }
