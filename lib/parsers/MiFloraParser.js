...
function decodeMeasurement(frame) {
  ...
  switch (frame.dataType) {
    ...
    case DATA_TYPES.ILLUMINANCE:
      ...
      return {
        luminance: payload.readUIntLE(0, 3),
      };
    ...
    case DATA_TYPES.CONDUCTIVITY:
      if (payload.length !== 2) return {};
      return { nutrition: payload.readUInt16LE(0) };

    case DATA_TYPES.BATTERY:
      if (payload.length !== 1) return {};
      return {
        battery: payload.readUInt8(0),
        batteryLow: payload.readUInt8(0) <= 20,
      };
...
function match(adv) {
  ...
  if (!frame) return { confidence: 0, reason: 'not-flower-care-mibeacon' };

  const localName = String(adv?.localName || '').toLowerCase();
  if (localName === 'flower care') {
    return { confidence: 100, reason: 'flower-care-name-product-service-data' };
  }

  return { confidence: 95, reason: 'flower-care-product-service-data' };
}
